...
World::World()
{
	...
	max_time = 60;
	time_count = max_time;
	sec_1.it_value.tv_sec = 1;
	sec_1.it_value.tv_nsec = 0;
	sec_1.it_interval.tv_sec = 1;
	sec_1.it_interval.tv_nsec = 0;
...
void World::readmap(string filename)
{
	...
	debug(to_string(range_x) + " : " + to_string(range_y) + " : " + to_string(LINES) + " : " + to_string(COLS));
	//cout << (range_x*5+3) << (range_y*10+2) << endl;
...
void World::mainmap()
{
	...
	time(&now);
 	...
 	timeinfo=localtime(&now);
	// double sec_ = ((double)(t-t_temp))/(double)CLOCKS_PER_SEC;
	// if(sec_ > 1.35 + (double(player_num_want-2)/2.0*0.10)){
	// 	time_count -= int(sec_);
	// 	t_temp = clock();
	// 	debug("-sec");
	// 	debug(to_string(time_count));
	// }
...
void get_time(struct sock_data *sock_data_p){
	time_t now;
	struct tm * timeinfo;
	time(&now);
 	timeinfo=localtime(&now);
	char alltime[256];
	strcpy(alltime, asctime(timeinfo));
	char nowtime[10], day[5], month[5], date[5], year[5], tmp[2];
	int hour = 0, min = 0, sec = 0;
	sscanf(alltime, "%s %s %s %d:%d:%d %s", day, month, date, &hour, &min, &sec, year);
	sprintf(sock_data_p->time_str, "%02d %02d", hour, min);
...
void World::setbomb(int i)
{
	if(p[i].drown==0&&p[i].dead==0){
		for(int j=0;j<p[i].amount;j++){
			if(p[i].bomb[j].set==0 && map[p[i].x][p[i].y].bptr==NULL){
	...
	for(int i=0;i<range_x;i++){
		for(int j=0;j<range_y;j++){
			if(map[i][j].bptr!=NULL&&map[i][j].bptr->bang==0){
				if(((float)(t-map[i][j].bptr->t))/CLOCKS_PER_SEC>=4.5){
...
struct point World::get_pos(int i){
	struct point xy;
	xy.x = p[i].x;
	xy.y = p[i].y;
	return xy;
